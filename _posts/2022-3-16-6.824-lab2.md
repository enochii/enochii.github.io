...
这里主要说下写 lab2 的心路历程。其实 2B 我还没“过”，现在主要是 `TestBackup2B` 这个用例一直卡着（10 次大概能过 7 次），后面我会单拎出来分析。所以这篇论文是从一个 loser 的角度给出一些不成熟的意见。

> PS：调完参数后，最近的一次测试是 24/30 。
...
这里我的做法是拉大 election timeout 的区间即 `[TIMEOUT_LEFT_MS, TIMEOUT_RIGHT_MS]`，增加心跳包的频率，给出如下参数做参考。一开始我看测试有个变量 `RaftElectionTimeout = 1000` ms ，所以我很克制 election timeout 的上限，后来想了下，理论上只要让中位数不太大就 OK 了。

> PS：鉴于这个测试有点苛刻，我个人觉得其实差不多就行... 不知道大家是怎么测的。
...
这门课还是很推荐的，也让我深感测试的重要性，尤其是对分布式来说。Have fun~
